...
namespace Admin.NET.Core.Utility
{
    public class NpoiExcelImportHelper
    {
        ...
        /// <returns></returns>
        public DataTable ExcelToDataTable(string filePath, out bool isSuccess, out string resultMsg)
        {
            using (var stream = File.OpenRead(filePath))
            {
                return ExcelToDataTable(stream, Path.GetExtension(filePath), out isSuccess, out resultMsg);
            }
        }
                    ...
                    for (int j = row.FirstCellNum; j < cellCount; j++)
                    {
                        if (row.GetCell(j) != null)//单元格内容非空验证
                        {
                            ...
                            switch (cell.CellType)
                            {
                                ...
                                //错误
                                case CellType.Error:
                                    dataRow[j] = "error"; // HSSFErrorConstants.GetText(row.GetCell(j).ErrorCellValue);
                                    break;
