...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysRoleMenu> _sysRoleMenuRep;  // 角色菜单表仓储
        private readonly ISimpleRepository<SysDbLocator, SysMenu> _sysMenuRep;  // 菜单表仓储
        private readonly ISysCacheService _sysCacheService;

        public SysRoleMenuService(ISimpleRepository<SysDbLocator, SysRoleMenu> sysRoleMenuRep, ISimpleRepository<SysDbLocator, SysMenu> sysMenuRep, ISysCacheService sysCacheService)
        {
        ...
        {
            var roleMenus = await _sysRoleMenuRep.Where(u => u.SysRoleId == input.Id).ToListAsync();
            await _sysRoleMenuRep.DeleteAsync(roleMenus);

            // 确保对按钮菜单的父级菜单能正确的授权，发现前端构造的数据会丢弃了按钮菜单的父级菜单
            var roleMenuPidsList = await _sysMenuRep.Where(u => input.GrantMenuIdList.Contains(u.Id)).ToListAsync(u => u.Pids);
        ...
        {
            var roleMenus = await _sysRoleMenuRep.Where(u => menuIdList.Contains(u.SysMenuId)).ToListAsync();
            await _sysRoleMenuRep.DeleteAsync(roleMenus);
        }
