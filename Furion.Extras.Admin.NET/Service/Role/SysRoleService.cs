...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysRole> _sysRoleRep;  // 角色表仓储
        private readonly ISimpleRepository<SysDbLocator, SysUserRole> _sysUserRoleRep;  // 用户角色表仓储
        ...
        public SysRoleService(ISimpleRepository<SysDbLocator, SysRole> sysRoleRep,
                              ISimpleRepository<SysDbLocator, SysUserRole> sysUserRoleRep,
        ...
        {
            return await _sysRoleRep.AsDatabase().Select<SysRole, SysUserRole>()
                           .InnerJoin((a, b) => a.Id == b.SysRoleId)
                           .Where((a, b) => b.SysUserId == userId)
                           .ToListAsync((a, b) => new RoleOutput
                           {
                               Id = a.Id,
                               Code = a.Code,
                               Name = a.Name
        ...
        public async Task<dynamic> GetRoleList([FromQuery] RoleInput input)
        {
            ...
            return await _sysRoleRep
                                    .Where(name, u => u.Name.Contains(input.Name.Trim()))
                                    .Where(name, u => u.Code.Contains(input.Code.Trim()))
        ...
        public async Task<dynamic> GetRoleDropDown()
        {
            ...
            var roles = _userManager.SuperAdmin
                        ? await _sysUserRoleRep.Where(u => u.SysUserId == _userManager.UserId).ToListAsync(u => u.SysRoleId)
                        : new List<long>();
        ...
        {
            var adminRole = await _sysRoleRep.FirstOrDefaultAsync(u => u.Id == input.Id);
            if (adminRole.Code == CommonConst.SYS_MANAGER_ROLE_CODE)
                throw Oops.Oh(ErrorCode.D1020);

            var isExist = await _sysRoleRep.AnyAsync(u => (u.Name == input.Name || u.Code == input.Code) && u.Id != input.Id);
            if (isExist)
                throw Oops.Oh(ErrorCode.D1006);

            var sysRole = input.Adapt<SysRole>();
            await _sysRoleRep.UpdateExcludeAsync(sysRole,new[] { nameof(SysRole.DataScopeType) }, true);
        }
