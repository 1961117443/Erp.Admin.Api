...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysRoleDataScope> _sysRoleDataScopeRep;  // 角色数据范围表仓储

        public SysRoleDataScopeService(ISimpleRepository<SysDbLocator, SysRoleDataScope> sysRoleDataScopeRep)
        {
        ...
        {
            var dataScopes = await _sysRoleDataScopeRep.Where(u => u.SysRoleId == input.Id).ToListAsync();
        ...
        {
            return await _sysRoleDataScopeRep.Where(u => roleIdList.Contains(u.SysRoleId))
                                             .ToListAsync(u => u.SysOrgId);
        }
        ...
        {
            var dataScopes = await _sysRoleDataScopeRep.Where(u => orgIdList.Contains(u.SysOrgId)).ToListAsync();
            await _sysRoleDataScopeRep.DeleteAsync(dataScopes);
        }
        ...
        {
            var dataScopes = await _sysRoleDataScopeRep.Where(u => u.SysRoleId == roleId).ToListAsync();
            await _sysRoleDataScopeRep.DeleteAsync(dataScopes);
        }
    }
}