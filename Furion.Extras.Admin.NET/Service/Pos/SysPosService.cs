...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysPos> _sysPosRep;  // 职位表仓储

        private readonly ISysEmpPosService _sysEmpPosService;
        private readonly ISysEmpExtOrgPosService _sysEmpExtOrgPosService;

        public SysPosService(ISimpleRepository<SysDbLocator, SysPos> sysPosRep,
        ...
        {
            var code = !string.IsNullOrEmpty(input.Code?.Trim());
            return await _sysPosRep.Where(code, u => EF.Functions.Like(u.Code, $"%{input.Code.Trim()}%"))
        ...
        public async Task UpdatePos(UpdatePosInput input)
        {
            ...
            var pos = input.Adapt<SysPos>();
            await _sysPosRep.UpdateAsync(pos,ignoreNullValues: true);
        }
