...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    public class SysMenuService : ISysMenuService, IDynamicApiController, ITransient
    {
        ...
        public SysMenuService(ISimpleRepository<SysDbLocator, SysMenu> sysMenuRep,
            ...
            if (permissions == null || permissions.Count < 1)
            {
                if (!_userManager.SuperAdmin && userId != 0)
                {
                    ...
                    permissions = await _sysMenuRep.Where(u => menuIdList.Contains(u.Id))
                                                ...
                                                .ToListAsync(u => u.Permission);
                }
                else
                {
                    permissions = await _sysMenuRep
                                                   .Where(u => u.Type == MenuType.BTN)
                                                   .Where(u => u.Status == CommonStatus.ENABLE)
                                                   .ToListAsync(u => u.Permission);
                }
        ...
        public async Task<dynamic> GetMenuList([FromQuery] GetMenuListInput input)
        {
            ...
            var menus = await _sysMenuRep.Where(application, u => u.Application == input.Application.Trim())
                                         .Where(name, u => u.Name == input.Name.Trim())
                                         .Where(u => u.Status == CommonStatus.ENABLE)
                                         .OrderBy(u => u.Sort)
                                         .ToListAsync<MenuOutput>();
        ...
        public async Task AddMenu(AddMenuInput input)
        {
            ...
            var menu = input.Adapt<SysMenu>(a => a.Create());
        ...
        public async Task UpdateMenu(UpdateMenuInput input)
        {
            ...
            {
                // 查找所有叶子节点，包含子节点的子节点
                var menuList = await _sysMenuRep.Where(u => u.Pids.Contains($"%{oldMenu.Id}%")).ToListAsync();
                // 更新所有子节点的应用为当前菜单的应用
                if (menuList.Count > 0)
                {
                    // 更新所有子节点的application
            ...
            // 更新当前菜单
            oldMenu = input.Adapt<SysMenu>(a => a.Modify());
            oldMenu.Pids = newPids;

            await _sysMenuRep.UpdateAsync(oldMenu, ignoreNullValues: true);
