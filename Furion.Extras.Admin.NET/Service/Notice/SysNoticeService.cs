...
namespace Furion.Extras.Admin.NET.Service.Notice
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysNotice> _sysNoticeRep;  // 通知公告表仓储
        private readonly ISimpleRepository<SysDbLocator, SysNoticeUser> _sysNoticeUserRep;  // 通知公告用户表仓储

        private readonly IUserManager _userManager;

        private readonly ISysNoticeUserService _sysNoticeUserService;

        public SysNoticeService(ISimpleRepository<SysDbLocator, SysNotice> sysNoticeRep,
                                ISimpleRepository<SysDbLocator, SysNoticeUser> sysNoticeUserRep,
        ...
        public async Task<dynamic> QueryNoticePageList([FromQuery] NoticePageInput input)
        {
            ...
            var notices = await _sysNoticeRep
                                             .Where(searchValue, u => u.Title.Contains(input.SearchValue.Trim()) || u.Content.Contains(input.SearchValue.Trim()))
        ...
        public async Task AddNotice(AddNoticeInput input)
        {
            ...
            var newItem = await _sysNoticeRep.InsertAsync(notice);
        ...
        public async Task<dynamic> ReceivedNoticePageList([FromQuery] NoticePageInput input)
        {
            ...
            var notices = await _sysNoticeRep.AsDatabase().Select<SysNotice, SysNoticeUser>()
                .InnerJoin((n, u) => n.Id == u.NoticeId)
              .Where((n, u) => u.UserId == _userManager.UserId)
              .WhereIf(!string.IsNullOrWhiteSpace(input.SearchValue), (n, u) => n.Title.Contains(input.SearchValue.Trim()) || n.Content.Contains(input.SearchValue.Trim()))
              .WhereIf(input.Type > 0, (n, u) => n.Type == input.Type)
              .ToPagedListAsync(input.PageNo, input.PageSize, (n, u) => new NoticeReceiveOutput
              {
                  ReadStatus = u.ReadStatus,
                  ReadTime = u.ReadTime
              });
