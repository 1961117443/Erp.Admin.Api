...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysUser> _sysUserRep;  // 用户表仓储
        ...
        public SysUserService(ISimpleRepository<SysDbLocator, SysUser> sysUserRep,
        ...
        public async Task<dynamic> QueryUserPageList([FromQuery] UserPageInput input)
        {
            ...
            var users = await _sysUserRep.AsDatabase().Select<SysUser, SysEmp, SysOrg, SysTenant>()
                .InnerJoin((u, e, o, t) => u.Id == e.Id)
                .InnerJoin((u, e, o, t) => e.OrgId == o.Id)
                .InnerJoin((u, e, o, t) => u.TenantId == t.Id)
                .WhereIf(!string.IsNullOrWhiteSpace(searchValue), (u, e, o, t) => u.Account.Contains(input.SearchValue.Trim()) ||
                                                                            u.Name.Contains(input.SearchValue.Trim()) ||
                                                                            u.Phone.Contains(input.SearchValue.Trim()))
                .WhereIf(!string.IsNullOrWhiteSpace(pid), (u, e, o, t) => e.OrgId == long.Parse(pid) ||
                                                                        o.Pids.Contains(pid.Trim()))
                .WhereIf(Enum.IsDefined(typeof(CommonStatus), input.SearchStatus), (u, e, o, t) => u.Status == input.SearchStatus)
                .WhereIf(!superAdmin, (u, e, o, t) => u.AdminType != AdminType.SuperAdmin)
                .WhereIf(!superAdmin && dataScopes.Any(), (u, e, o, t) => dataScopes.Contains(e.OrgId))
                .ToPagedListAsync(input.PageNo, input.PageSize, (u, e, o, t) => new UserOutput
                {
                    Id = u.Id.ToString(),
                    Account = u.Account,
                    Tel = u.Account,
        ...
        public async Task AddUser(AddUserInput input)
        {
            ...
            CheckDataScope(input.SysEmpParam.OrgId);
             ...
             var isExist = await _sysUserRep.AnyAsync(u => u.Account == input.Account && !u.IsDeleted);
            ...
            var newUser = await _sysUserRep.InsertAsync(user);
        ...
        public async Task UpdateUser(UpdateUserInput input)
        {
            ...
            var user = input.Adapt<SysUser>();
            await _sysUserRep.UpdateExcludeAsync(user,new[] { nameof(SysUser.Password), nameof(SysUser.Status), nameof(SysUser.AdminType) }, true);
        ...
        {
            var user = await _sysUserRep.FirstOrDefaultAsync(u => u.Id == id);
        ...
        {
            var user = input.Adapt<SysUser>();
            await _sysUserRep.UpdateExcludeAsync(user,new[] { nameof(SysUser.AdminType), nameof(SysUser.LastLoginIp), nameof(SysUser.LastLoginTime) });
        }
        ...
        {
            var users = _sysUserRep.AsQueryable();
