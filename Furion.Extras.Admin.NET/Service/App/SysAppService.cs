...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysApp> _sysAppRep;    // 应用表仓储

        private readonly IUserManager _userManager;
        private readonly ISysMenuService _sysMenuService;

        public SysAppService(ISimpleRepository<SysDbLocator, SysApp> sysAppRep,
        ...
        {
            var apps = _sysAppRep.Entities.Where(u => u.Status == CommonStatus.ENABLE);
            if (!_userManager.SuperAdmin)
            {
                var appCodeList = await _sysMenuService.GetUserMenuAppCodeList(userId);
                apps = apps.Where(u => appCodeList.Contains(u.Code));
            }
            var appList = await apps.OrderBy(u => u.Sort).ToListAsync(u => new AppOutput
            {
                Code = u.Code,
                Name = u.Name,
                Active = u.Active
            }); // .OrderByDescending(u => u.Active) // 将激活的放到第一个
        ...
        public async Task<dynamic> QueryAppPageList([FromQuery] AppPageInput input)
        {
            ...
            var apps = await _sysAppRep.Entities
                                       .Where(name, u => u.Name.Contains(input.Name.Trim()))
                                       .Where(code, u => u.Code.Contains(input.Code))
                                       .OrderBy(u => u.Sort)
        ...
        {
            var isExist = await _sysAppRep.Entities.AnyAsync(u => u.Name == input.Name || u.Code == input.Code);
            if (isExist)
                throw Oops.Oh(ErrorCode.D5000);

            if (input.Active == YesOrNot.Y.ToString())
            {
                isExist = await _sysAppRep.Entities.AnyAsync(u => u.Active == input.Active);
                if (isExist)
                    throw Oops.Oh(ErrorCode.D5001);
            }

            var app = input.Adapt<SysApp>(a => a.Create());
        ...
        {
            var isExist = await _sysAppRep.Entities.AnyAsync(u => (u.Name == input.Name || u.Code == input.Code) && u.Id != input.Id);
            if (isExist)
                throw Oops.Oh(ErrorCode.D5000);

            if (input.Active == YesOrNot.Y.ToString())
            {
                isExist = await _sysAppRep.Entities.AnyAsync(u => u.Active == input.Active && u.Id != input.Id);
                if (isExist)
                    throw Oops.Oh(ErrorCode.D5001);
            }

            var app = input.Adapt<SysApp>();
            await _sysAppRep.UpdateExcludeAsync(app,new[] { nameof(SysApp.Status) }, true);
        }
        ...
        {
            return await _sysAppRep.Entities.FirstOrDefaultAsync(u => u.Id == input.Id);
        }
        ...
        {
            return await _sysAppRep.Entities.Where(u => u.Status == CommonStatus.ENABLE).OrderBy(u => u.Sort).ToListAsync();
        }
