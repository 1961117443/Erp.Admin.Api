...
namespace Furion.Extras.Admin.NET.Service
{
        ...
        where TImportDto : class, new()
    {
        ...
        /// 数据仓储
        /// </summary>
        protected readonly IFreeSqlRepository<TEntity> Repository;

        /// <summary>
        /// 构造函数
        /// </summary>
        /// <param name="repository"></param>
        public BaseService(IFreeSqlRepository<TEntity> repository)
        {
        ...
        /// 自定义分页搜索（复杂查询）
        /// </summary>
        protected Func<TSearchDto, ISelect<TEntity>> SearchQueryable = null;

        /// <summary>
        /// 分页数据返回前处理
        /// </summary>
        /// <returns></returns>
        protected Action<FreeSqlPagedList<TEntity>> PageListHandle = null;
        ...
        public virtual async Task<PageResult<TPageListDto>> PageList(TSearchDto searchDto)
        {
            ...
            else
            {
                ...
                queryable = Repository.AsQueryable();
                // 有自定义的查询条件
                if (SearchExpression != null)
                    queryable = queryable.Where(SearchExpression(searchDto));
            }

            FreeSqlPagedList<TEntity> pageList = await queryable.ToPagedListAsync(searchDto.PageNo, searchDto.PageSize);
        ...
        public virtual async Task Add(TAddDto addDto)
        {
            ...
            // 写数据
            var entity = await Repository.InsertAsync(addDto.Adapt<TEntity>());
        ...
        public virtual async Task Delete(List<long> ids)
        {
            ...
            var count = await Repository.DeleteAsync(x => ids.Contains(x.Id));
        ...
        public virtual async Task FakeDelete(List<long> ids)
        {
            ...
            var count = await Repository.SoftDeleteAsync(x => ids.Contains(x.Id));
            //var count = await Repository.AsUpdateable()
        ...
        public virtual async Task Update(TUpdateDto updateDto)
        {
            ...
            await Repository.UpdateAsync(entity, ignoreNullValues: true);
            AfterUpdateAction?.Invoke(entity);
        }
        ...
        /// 自定义导出搜索（复杂查询）
        /// </summary>
        protected Func<TSearchDto, ISelect<TEntity>> ExportSearchQueryable = null;
        ...
        public virtual async Task<FileContentResult> Export(TSearchDto searchDto)
        {
            ...
            else
            {
                // 动态构建查询条件
                GetSearchParameters(searchDto);
                //queryable = Repository.Search(searchDto);
                queryable = Repository.AsQueryable();
                // 有自定义的查询条件
