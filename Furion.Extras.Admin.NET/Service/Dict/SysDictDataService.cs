...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysDictData> _sysDictDataRep;  // 字典类型表仓储
        private readonly IUserManager _userManager;

        public SysDictDataService(ISimpleRepository<SysDbLocator, SysDictData> sysDictDataRep, IUserManager userManager)
        {
        ...
        public async Task<dynamic> QueryDictDataPageList([FromQuery] DictDataPageInput input)
        {
            ...
            var dictDatas = await _sysDictDataRep
                                  ...
                                  .Where(value, u => u.Value.Contains(input.Value.Trim()))
                                  .Where(u => (u.Status != CommonStatus.DELETED && !supperAdmin) || (u.Status <= CommonStatus.DELETED && supperAdmin)).OrderBy(u => u.Sort)
                                  .ToPagedListAsync<SysDictData, DictDataOutput>(input.PageNo, input.PageSize);
        ...
        {
            return await _sysDictDataRep.Where(u => u.TypeId == input.TypeId).Where(u => u.Status != CommonStatus.DELETED).OrderBy(u => u.Sort).ToListAsync();
        }
        ...
        public async Task<dynamic> GetDictDataListByDictTypeId(long dictTypeId)
        {
            return await _sysDictDataRep.AsQueryable()
                           .LeftJoin(u => u.SysDictType.Id == u.TypeId)
                           .Where(u => u.SysDictType.Id == dictTypeId)
