...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysOrg> _sysOrgRep;  // 组织机构表仓储
        ...
        public SysOrgService(ISimpleRepository<SysDbLocator, SysOrg> sysOrgRep,
        ...
        public async Task<dynamic> QueryOrgPageList([FromQuery] OrgPageInput input)
        {
            ...
            var orgs = await _sysOrgRep.Where(name, u => u.Name.Contains(input.Name.Trim())) // 根据机构名称模糊查询
                                       .Where(id, u => u.Id == long.Parse(input.Id.Trim())) // 根据机构id查询
                                       .Where(pId, u => u.Pids.Contains($"%{input.Pid.Trim()}%") || u.Id == long.Parse(input.Pid.Trim())) // 根据父机构id查询
                                       .Where(dataScopeList.Count > 0, u => dataScopeList.Contains(u.Id)) // 非管理员范围限制
                                       .Where(u => u.Status != CommonStatus.DELETED).OrderBy(u => u.Sort)
                                       .ToPagedListAsync<SysOrg, OrgOutput>(input.PageNo, input.PageSize);
        ...
        private List<long> GetDataScopeList(List<long> dataScopes)
        {
            ...
            if (!_userManager.SuperAdmin)
            {
                ...
                {
                    var sysOrg = _sysOrgRep.Get(c => c.Id == u);
        ...
        public async Task AddOrg(OrgAddInput input)
        {
            ...
            var newOrg = await _sysOrgRep.InsertAsync(sysOrg);
            ...
            if (App.User.FindFirst(ClaimConst.CLAINM_SUPERADMIN)?.Value != ((int)AdminType.SuperAdmin).ToString())
            {
                ...
                await _sysOrgRep.Change<SysUserDataScope>().InsertAsync(new SysUserDataScope
                {
                    SysUserId = long.Parse(userId),
                    SysOrgId = newOrg.Id
                });
                dataScopes.Add(newOrg.Id);
        ...
        public async Task DeleteOrg(DeleteOrgInput input)
        {
            ...
            // 该机构下面子机构若有员工，则不能删
            var orgIds = await _sysOrgRep.Where(u => u.Pids.Contains(input.Id)).ToListAsync(u => u.Id);
        ...
        public async Task UpdateOrg(UpdateOrgInput input)
        {
            ...
            await FillPids(sysOrg);
            await _sysOrgRep.UpdateAsync(sysOrg,ignoreNullValues: true);
        ...
        public async Task<dynamic> GetOrgTree()
        {
            ...
            var orgs = await _sysOrgRep.Where(dataScopeList.Count > 0, u => dataScopeList.Contains(u.Id))
                                                        .Where(u => u.Status == CommonStatus.ENABLE).OrderBy(u => u.Sort)
                                                        .ToListAsync(u => new OrgTreeNode
                                                        {
                                                            Id = u.Id,
                                                            ParentId = u.Pid,
                                                            Title = u.Name,
        ...
        public async Task<List<long>> GetDataScopeListByDataScopeType(int dataScopeType, long orgId)
        {
            ...
            {
                orgIdList = await _sysOrgRep.Where(u => u.Status == CommonStatus.ENABLE).ToListAsync(u => u.Id);
            }
        ...
        {
            return await _sysOrgRep.AsQueryable().ToListAsync(u => u.Id);
        }
