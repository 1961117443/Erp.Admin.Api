...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<LogDbLocator, SysLogOp> _sysOpLogRep; // 操作日志表仓储

        public SysOpLogService(ISimpleRepository<LogDbLocator, SysLogOp> sysOpLogRep)
        {
        ...
        public async Task<dynamic> QueryOpLogPageList([FromQuery] OpLogPageInput input)
        {
            ...
            var opLogs = await _sysOpLogRep.Where(name, u => u.Name.Contains(input.Name.Trim()))
                                           .Where(success, u => u.Success == input.Success)
                                           .Where(ReqMethod, u => u.ReqMethod == input.ReqMethod)
                                           .Where(searchBeginTime, u => u.OpTime >= DateTime.Parse(input.SearchBeginTime.Trim()) && u.OpTime <= DateTime.Parse(input.SearchEndTime.Trim()))
                                           .OrderBy(u => u.Id)
                                           .ToPagedListAsync<SysLogOp, OpLogOutput>(input.PageNo, input.PageSize);
