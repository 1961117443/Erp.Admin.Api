...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    public class AuthService : IAuthService, IDynamicApiController, ITransient
    {
        ...
        public AuthService(ISimpleRepository<SysDbLocator, SysUser> sysUserRep,
        ...
        public string LoginAsync([Required] LoginInput input)
        {
            ...
            var user = _sysUserRep.Get(u => u.Account.Equals(input.Account) && u.Password.Equals(encryptPasswod) && !u.IsDeleted);
        ...
        public async Task<LoginOutput> GetLoginUserAsync()
        {
            ...
            // 更新用户最后登录Ip和时间
            await _sysUserRep.UpdateIncludeAsync(user, a=> new[] { nameof(SysUser.LastLoginIp), nameof(SysUser.LastLoginTime) });
