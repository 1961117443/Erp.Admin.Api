...
namespace Furion.Extras.Admin.NET.Service
{
    ...
    {
        private readonly ISimpleRepository<SysDbLocator, SysTimer> _sysTimerRep;  // 任务表仓储
        private readonly ISysCacheService _cache;

        public SysTimerService(ISimpleRepository<SysDbLocator, SysTimer> sysTimerRep, ISysCacheService cache)
        {
        ...
        public async Task<dynamic> GetTimerPageList([FromQuery] JobPageInput input)
        {
            ...
            var timers = await _sysTimerRep
                .Where(!string.IsNullOrEmpty(input.JobName?.Trim()), u => u.JobName.Contains(input.JobName.Trim()))
                .ToPagedListAsync<SysTimer, JobOutput>(input.PageNo, input.PageSize);
        ...
        public async Task UpdateTimber(UpdateJobInput input)
        {
            ...
            var timer = input.Adapt<SysTimer>();
            await _sysTimerRep.UpdateAsync(timer,ignoreNullValues: true);
        ...
        {
            var sysTimerList = _sysTimerRep.Entities.Where(t => t.StartNow).ToList<AddJobInput>();
